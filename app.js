...
function renderGenerales(keep) {
  ...
  content.innerHTML = `<div class="screen">
    ...
    ${genSub === 'mapa' ? '<div style="display:flex;gap:8px;align-items:center;margin-bottom:8px;flex:0 0 auto;padding:0 2px"><label style="display:flex;align-items:center;gap:7px;font-size:13px;font-weight:700;color:var(--muted);cursor:pointer"><input type=\"checkbox\" id=\"chkSoloCasos\" ' + (showOnlyCases ? 'checked' : '') + '> Solo establecimientos con casos</label></div>' : ''}
    <div class="mkpis">
...
function drawDensity(map, layer) {
  ...
  if (!showOnlyCases) {
    for (const est of ESTS) {
      ...
      mk.bindPopup(`<b>${esc(est.nom)}</b><br>RBD ${esc(est.rbd)} · ${esc(est.com)}<div class="popup-cases" style="color:#888">Sin casos activos</div>`);
    }
  }
  ...
  for (const rbd in byRbd) {
    ...
    mk.on('popupopen', e => { const a = e.popup.getElement().querySelector('.popup-open'); if (a) a.onclick = ev => { ev.preventDefault(); const e2 = estByRbd(rbd); if (e2) openEstablecimiento(e2); }; });
