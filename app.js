...
function renderGenerales(keep) {
  ...
  if (!keep) { genSub = 'lista'; genKpi = null; }
  ...
  content.innerHTML = `<div class="screen">
    ${esMapa ? '' : `<div class="search-wrap" id="genSearchWrap">
    ...
    ${esMapa ? '<div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;flex:0 0 auto;padding:0 2px"><label style="display:flex;align-items:center;gap:7px;font-size:12.5px;font-weight:700;color:var(--muted);cursor:pointer"><input type=\"checkbox\" id=\"chkSoloCasos\" ' + (showOnlyCases ? 'checked' : '') + '> Solo establecimientos con casos</label></div>' : ''}
    <div class="mkpis ${esMapa ? 'flat' : ''}">
      <button class="mkpi ${genKpi === 'act' ? 'ksel' : ''}" data-k="act"><div class="bar"></div><div class="n">${total}</div><div class="l">Activos</div></button>
      <button class="mkpi b ${genKpi === 'der' ? 'ksel' : ''}" data-k="der"><div class="bar"></div><div class="n">${derivados}</div><div class="l">Derivados</div></button>
      <button class="mkpi ${genKpi === 'novis' ? 'ksel' : ''}" data-k="novis"><div class="bar"></div><div class="n">${noVis}</div><div class="l">No visados</div></button>
    </div>
    ${!esMapa && genKpi ? `<div class="kpi-filter-note">Filtrando: <b>${genKpi === 'act' ? 'activos' : (genKpi === 'der' ? 'derivados' : 'no visados')}</b> · <span id="clrGenKpi" style="text-decoration:underline;cursor:pointer">quitar</span></div>` : ''}
    <div id="genBody" style="flex:1 1 auto;min-height:0;display:flex;flex-direction:column">${esMapa ? `<div class="mapwrap" id="mapwrap"><div id="map"></div>${mapLegend()}</div>` : `<div class="caselist" id="genList"></div>`}</div>
  </div>`;

  $$('.seg button').forEach(b => b.onclick = () => { genSub = b.dataset.s; genKpi = null; renderGenerales(true); });
  $$('.mkpi[data-k]').forEach(k => k.onclick = () => {
    if (esMapa) { genSub = 'lista'; genKpi = k.dataset.k; renderGenerales(true); return; }
  ...
  if (esMapa) {
    setTimeout(() => { const chk = $('#chkSoloCasos'); if (chk) chk.onchange = () => { showOnlyCases = chk.checked; if (mapObj) drawDensity(mapObj, mapLayer); }; }, 50);
    setTimeout(() => initMap('map'), 60);
  } else {
...
function paintGenList() {
  ...
  else if (genKpi === 'der') list = list.filter(r => estadoDe(r).k === 'der');
  else if (genKpi === 'novis') list = list.filter(r => estadoDe(r).k === 'pend');
