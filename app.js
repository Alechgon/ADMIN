...
const DEFAULT_EXEC = 'https://script.google.com/macros/s/AKfycbxv3DPb9LUoP_fCKZPuZmadlR8qk22q-lbjVU7q-apcF3Zff-TkSxoWZYX4C3ZC2pK5/exec';
...
async function subscribePush() {
  try {
    if (!CFG.pushUrl) return false;
    if (!('serviceWorker' in navigator) || !('PushManager' in window)) { toast('Este navegador no soporta push'); return false; }
    await askNotifPermission();
    if (Notification.permission !== 'granted') { toast('Debes permitir notificaciones'); return false; }
    const reg = await navigator.serviceWorker.ready;
    // obtener la llave pública VAPID del servidor
    const kr = await fetch(CFG.pushUrl.replace(/\/$/, '') + '/vapidPublicKey');
    ...
    const res = await fetch(CFG.pushUrl.replace(/\/$/, '') + '/subscribe', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ subscription: sub, sheetUrl: CFG.sheetUrl })
    });
    const d = await res.json().catch(() => ({}));
    if (d && d.ok !== false) { saveCfg(Object.assign({}, CFG, { pushOn: true })); return true; }
    return false;
