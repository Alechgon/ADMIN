...
const LS_CACHE = 'soser_admin_cache_v1';   // caché local de los datos (render instantáneo)
const CACHE_MAX_AGE = 1000 * 60 * 60 * 24; // 24h: si es más viejo, igual se muestra pero avisa
...
async function fetchAdmin() {
  ...
  try {
    ...
    const to = setTimeout(() => ctrl.abort(), 20000);  // corta a los 20s en vez de esperar indefinido
    const res = await fetch(CFG.sheetUrl + '?admin=1&t=' + Date.now(), { signal: ctrl.signal });
    clearTimeout(to);
...
/* ---------------- Caché local (arranque instantáneo, sin esperar la red) ---------------- */
const CACHE_KEY = 'soser_admin_cache_v1';
function saveCache() {
  try {
    localStorage.setItem(CACHE_KEY, JSON.stringify({
      ts: Date.now(), reportes: ALL, tecnicos: TECNICOS, coords: SERVER_COORDS
    }));
...
function loadCache() {
  try {
    ...
    if (c && Array.isArray(c.reportes)) return c;
  } catch (e) {}
  return null;
}
function cacheAgeTxt(ts) {
  if (!ts) return '';
...
async function fetchCoords(withGeo) {
  if (!CFG.sheetUrl) return null;
  try { const r = await fetch(CFG.sheetUrl + '?coords=1' + (withGeo ? '&geo=1' : '') + '&t=' + Date.now()); const d = await r.json(); if (d && d.ok && d.coords) { SERVER_COORDS = d.coords; saveCache(); return d.coords; } } catch (e) {}
  return null;
}
...
async function refreshData(silent) {
  ...
  saveCache();               // persistir para el próximo arranque
  return true;
}
function startPolling() {
  stopPolling();
  pollTimer = setInterval(async () => {
    // no refrescar mientras el usuario está en Configuración o con un modal abierto
    if (currentView === 'config' || overlays.children.length) return;
...
function renderHome() {
  ...
  const _c = loadCache();
  const _cacheInfo = !cfgOk ? 'Sin conexión configurada'
    : (ALL.length ? `${ALL.length} casos · actualizado ${_c ? cacheAgeTxt(_c.ts) : 'ahora'}` : 'Cargando desde tu planilla…');
  ...
  content.innerHTML = `<div class="screen"><div style="flex:1;overflow-y:auto">
    ...
    <p class="note" style="text-align:center;margin-bottom:16px">${_cacheInfo}</p>
  </div></div>`;
...
function rerenderCurrent() {
  // nunca re-renderizar pantallas donde el usuario está escribiendo o navegando activamente
  if (currentView === 'config' || currentView === 'caso' || overlays.children.length) return;
  if (currentView === 'home') { updateHomeNumbers(); }
  else if (currentView === 'generales') {
    // si estamos en el mapa, solo actualizar los puntos (no recrear Leaflet: se sentiría como un salto)
    if (genSub === 'mapa' && mapObj && mapLayer && !mapPicked.length) drawDensity(mapObj, mapLayer);
    else if (genSub !== 'mapa') renderGenerales(true);
  }
...
function renderConfig() {
  currentView = 'config'; stopPolling();   // evita que el refresco automático te saque de aquí
  showNav(true); $('#btnHome').classList.remove('hidden'); $('#btnHome').onclick = () => { startPolling(); renderHome(); };
  ...
  $('#btnNext').onclick = () => {
    ...
    const urlCambio = url !== (CFG.sheetUrl || '');
    saveCfg(Object.assign({}, CFG, { sheetUrl: url, pushUrl: push })); toast('Configuración guardada');
    if (urlCambio) { try { localStorage.removeItem(CACHE_KEY); } catch (e) {} ALL = []; }  // caché viejo era de otra planilla
    refreshData(false).then(() => { fetchCoords(true); startPolling(); renderHome(); });
  };
...
function renderBitacora() {
  ...
  // si aún no llegan datos del exec, mostrar carga (evita tarjetas vacías)
  if (!ALL.length) {
    content.innerHTML = `<div class="screen">
      <div class="sub-nav"><button class="der-home" id="bitHome">🏠 Inicio</button></div>
      <div class="enc-title">📗 Bitácora de trabajos</div>
      <div class="loading" style="padding:60px 20px;text-align:center"><span class="spinner"></span><p style="color:var(--muted);margin-top:12px">Cargando casos…</p></div>
    </div>`;
...
/* ============================================================
   ...
   ============================================================ */
(async function init() {
  // 1) Pintar YA con lo último visto (caché), sin esperar la red -> arranque instantáneo
  const cached = loadCache();
  if (cached) {
    ALL = cached.reportes || [];
    if (Array.isArray(cached.tecnicos) && cached.tecnicos.length) TECNICOS = cached.tecnicos;
    if (cached.coords) SERVER_COORDS = cached.coords;
  }
  renderHome();

  // 2) Registrar service worker (no bloquea)
  ...
  // 3) Actualizar en segundo plano (sin trabar la interfaz)
  if (CFG.sheetUrl) {
    refreshData(true).then(ok => {
      if (ok && currentView === 'home') renderHome();
      startPolling();
    });
    fetchCoords(true);
    fetchTecnicos().then(t => { if (t && t.length) { TECNICOS = t; saveCache(); } });
  }
})();
